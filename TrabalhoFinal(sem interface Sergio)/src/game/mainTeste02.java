...
public class MainTeste {
	...
	public static void initiatePlay(Kingdom initialLoc, int initialCoins, int initialPowerGem, int initialPowerLimit,
			List gameMissions, Kingdom gameFinalLoc) {
		...
		while (true) {
			...
			if ((currentLoc == mission1.missionOrigin)) {
				...
				if (mission1.isComplete == false) {
					// TESTE PARA SABER SE HÁ UMA MISSÃO EM ANDAMENTO PARA APARECER A OPÇÃO DE CANCELAMENTO
					if (inMission == true) {
						...
						if (cancelMission == 1) {// MISSÃO EM ANDAMENTO CANCELADA
							...
							if (choiceMission == 1) {// MISSÃO ACEITA
								if (currentLoc.isJaPassou() == true) {// SE NÃO É A PRIMEIRA VEZ QUE ESTÁ PASSANDO PELO
																		// REINO
									System.out.println("Missão aceita!");
									inMission = true;
								}
								if (currentLoc.isJaPassou() == false) {// SE É A PRIMEIRA VEZ QUE ESTÁ PASSANDO PELO
																		// REINO
