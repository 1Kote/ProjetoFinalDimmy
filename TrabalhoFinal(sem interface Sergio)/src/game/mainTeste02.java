...
public class MainTeste {
	...
	public static void initiatePlay(Kingdom initialLoc, int initialCoins, int initialPowerGem, int initialPowerLimit, List gameMissions, Kingdom gameFinalLoc) {
		...
		while (true) {
			...
			System.out.println("Localização atual: " + currentLoc.getNameK()); //PRINT DA LOCALIZAÇÃO ATUAL
			System.out.println("Total de moedas: " + coins);//PRINT DO TOTAL DE MOEDAS
			System.out.println("Poder da joia: " + powerGem + "/" + powerLimit );//PRINT DO PODER DA JOIA E SEU LIMIAR
			if(inMission == true) {
				System.out.println("Missão em andamento: SIM");//PRINT PARA MOSTRAR SE HÁ UMA MUSSÃO EM ANDAMENTO
			}
			if(inMission == false) {
				System.out.println("Missão em andamento: NÂO");//PRINT PARA MOSTRAR SE HÁ UMA MUSSÃO EM ANDAMENTO
			}
			...
			if (currentLoc != initialLoc) {
				...
				System.out.println("MERCADOR: Ola, quantas Moedas vc possui?");
				int input = scn.nextInt();
				
				if (input != coins) {
					System.out.println("MERCADOR: O mercador também é um mago estudioso da clarividencia e sabe que você está mentindo!");
				...
				System.out.println("MAXUEL: Eu venho de " + pastLoc.getNameK() + " e vou para:" );
